...
static void
TcpTraces(uint32_t serverId, std::string pathVersion, std::string finalPart)
{
  AsciiTraceHelper asciiTraceHelper;

  std::cout << "***Mpika TCP traces***\n";

  std::ostringstream pathCW;
  pathCW << "/NodeList/" << serverId << "/$ns3::TcpL4Protocol/SocketList/0/TcpSocketBase/CongestionWindow";
  ...
  pathRTT << "/NodeList/" << serverId << "/$ns3::TcpL4Protocol/SocketList/0/TcpSocketBase/RTT";

  std::ostringstream fileRTT;
  fileRTT << pathVersion << "Tcp-rtt"  << serverId << finalPart;

  std::ostringstream pathRCWnd;
  pathRCWnd << "/NodeList/" << serverId << "/$ns3::TcpL4Protocol/SocketList/0/TcpSocketBase/RWND";

  std::ostringstream fileRCWnd;
  fileRCWnd <<pathVersion << "Tcp-rwnd-change"  << serverId << finalPart;

  std::ostringstream fileName;
  fileName << pathVersion << "Tcp-rx-data" << serverId << finalPart;

  std::ostringstream pathRx;
  pathRx << "/NodeList/" << serverId << "/$ns3::TcpL4Protocol/SocketList/*/TcpSocketBase/Rx";
...
int main(int argc, char *argv[]) {
	...
	Simulator::Schedule(Seconds(0.1), &ThroughputMonitor, &flowHelper, flowMonitor, tcpIpIfaces, quicIpIfaces, streamThroughput);*/
    ...
    for(std::map<FlowId, FlowMonitor::FlowStats>::const_iterator iter = stats.begin(); iter != stats.end(); ++iter) {
    	...
    	if(t.destinationPort == dlPort_tcp) {
    		//dlThroughput_tcp += (1000000*8.0)/(iter->second.timeLastRxPacket.GetSeconds()-iter->second.timeFirstTxPacket.GetSeconds()) / 1000000;
    	...
    	if(t.destinationPort == dlPort_quic) {
    		//dlThroughput_quic += (1000000*8.0)/(iter->second.timeLastRxPacket.GetSeconds()-iter->second.timeFirstTxPacket.GetSeconds()) / 1000000;
